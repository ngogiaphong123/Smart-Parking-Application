...
public class LightBulkService {
    ...
    public String turnOnOffLightBulk(String value) {
        ...
        try {
            Call<ApiRecord> call = AdafruitRetrofitClientAPI.getAdafruitApi().postToAdafruitFeed("cambien1", postToButton);
    ...
    public String getDeviceStatus() {
        try {
            Call<Set<ApiRecord>> call = AdafruitRetrofitClientAPI.getAdafruitApi().getRecordFromAdafruit("cambien1", 1);
            Set<ApiRecord> deviceStatus = call.execute().body();
            if (deviceStatus != null) {
                ApiRecord buttonRecord = deviceStatus.iterator().next();
